...
var parentId=$(this).data('comment');
  ...
  if (!$(this).next().hasClass('mtb')) {


 $(this).after('<div class="mtb"><form method="post" action=""> {{csrf_field()}} <input type="hidden" name="user_id" value="{{Auth::user()->id}}">  <input type="hidden" name="products" value="{{$id}}"> <input type="hidden" class="parent_id " name="parent_id" value="'+parentId+'"> <textarea class="form-control" placeholder="نظر خود را بنویسید" name="comment" id="comment'+parentId+'" ></textarea><input type="button" name="" value="ارسال" class="btn btn-primary" onclick="sendcomment({{Auth::user()->id}},{{$id}},'+parentId+')"></form></div>');
...
function sendcomment(id,products,parentId){
